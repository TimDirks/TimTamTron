...
function showFortune(message){
    Guild.findOneOrCreate({guildId: message.guild.id}, function(err, guild){
        ...
        let fortunes = "```List of all your fortunes```\n";
...
function addFortune(message, args){
    if(args.length === 0) {
        return message.channel.send("Please give me a fortune to add.");
    }
    ...
    Guild.findOneAndUpdate({guildId: message.guild.id}, {$push: {magicBall: newFortune}}, function (err, guild) {
        if(err) {
            return message.channel.send("Could not add fortune to the list!");
        }
    });
...
function resetFortune(message){
    Guild.reset8ball({guildId: message.guild.id}, function(err, guild) {
        if (err) {
            return message.channel.send("Something went wrong with reseting the fortunes for your guild.");
        }
